...
function controller(canvasId, options) {
  ...
  this.parentHost = options.parentHost;
...
controller.prototype = {
  ...
  drawControllerPart: function() {
    ...
    var my_gradient=ctx.createLinearGradient(0,0,0,region.height);
    my_gradient.addColorStop(0,"#769EFF");
    my_gradient.addColorStop(1,"#A8CFFF");
    ctx.fillStyle=my_gradient;

    //ctx.fillStyle = '#6c6c28'; //'#6c6c28';  background-image: linear-gradient(-180deg, #769EFF 0%, #A8CFFF 100%);
    ctx.globalAlpha = 1;//.5;
    ...
    ctx.beginPath();
    var leftBarRegion = {
      x: region.x + leftBarPosition - bar.width / 2,
      y: region.y + region.height / 2 - bar.height / 2,
      width: bar.width,
      height: bar.height
    };
    ...
    ctx.beginPath();
    var rightBarRegion = {
      x: region.x + rightBarPosition - bar.width / 2,
      y: region.y + region.height / 2 - bar.height / 2,
      width: bar.width,
      height: bar.height
    };
    ...
    if(me.isTouchDevice) {
      ...
      addEvent(canvas, 'touchend', function(e) {
        ...
        if(me.fingers && me.fingers.length) {
          ...
          if(timeSpan < window.tapTimeLimit && me.startTouch) {
            ...
            var point = {
              offsetX: evt.pageX - canvasPosition.x,
              offsetY: evt.pageY - canvasPosition.y
            };
      ...
      addEvent(canvas, 'touchstart', function(e) {
        ...
        if(touches.length) {
          for(var i = 0; i < touches.length; i++) {
            var touch = touches[i];
            var point = {
              offsetX: touch.pageX - canvasPosition.x,
              offsetY: touch.pageY - canvasPosition.y
            };
      ...
      var moveHandle = function(ev) {
        ...
        if(me.triggerBar) {
          ...
          if(typeof me.onPositionChanged == 'function' && me.isValueChanged()) {
            me.value = me.getValue();
            //me.onPositionChanged(me.value);   // 移动的时候，就不要实时刷新了，除非鼠标抬手才刷新
          }
          me.drawControllerPart();
        }
      };
      ...
      var startHandle = function(ev) {
        ...
        if(isOnLeftBar) me.triggerBar = {
          type: 'left',
          x: ev.offsetX,
          position: me.leftBarPosition
        };
        else if(isOnRightBar) me.triggerBar = {
          type: 'right',
          x: ev.offsetX,
          position: me.rightBarPosition
        };
        else if(isOnMiddle) me.triggerBar = {
          type: 'middle',
          x: ev.offsetX,
          leftPosition: me.leftBarPosition,
          rightPosition: me.rightBarPosition
      ...
      addEvent(me.canvas, 'mouseout', endMoveAndOut);
